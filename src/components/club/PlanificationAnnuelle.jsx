...
function ScannerDocument({ categorie, onExtrait, onAnnuler }) {
  ...
  const analyser = async () => {
    ...
    setError(null)
    try {
      const { data, error } = await supabase.functions.invoke('scan-planification', {
        body: { imageBase64: base64, mimeType: file.type, categorie, saison: saisonActuelle() },
      })
      if (error || data?.error) throw new Error(error?.message || data?.error)
      onExtrait(data.resultat)
    } catch (e) {
