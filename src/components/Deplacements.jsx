...
export default function Deplacements({ clubId, accentColor = '#4ade80', readOnly = false }) {
  ...
  const completerHorairesDepuisMatchs = async (liste) => {
    ...
    for (const d of incomplets) {
      ...
      // nb_personnes : ne complète que si absent (ne touche jamais une valeur
      // déjà saisie à la main, qui reflète le nombre réel de convoqués).
      updates.push({
        id: d.id, educateur_id: d.educateur_id || m.educateur_id || null, ville_destination: m.ville,
        ...
        ...(d.nb_personnes == null ? { nb_personnes: effectifParDefautMatch(m.competition) } : {}),
      })
    }
