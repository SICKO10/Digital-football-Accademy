...
export default function StatCard({ label, value, sub, subColor, accent, color, onClick, active = false }) {
  ...
  return (
    <div
      ...
      {sub && <p style={{ margin: 0, fontSize: '12px', color: subColor || c.textMuted, fontWeight: subColor ? 600 : 400 }}>{sub}</p>}
    </div>
  )
}
