  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      {[0.25, 0.5, 0.75, 1].map(l => (
        <polygon key={l} points={poly(l)} fill="none" stroke={l === 1 ? colors.border.strong : "#1e1e1e"} strokeWidth={l === 1 ? 1 : 0.5} />
      ))}
      {stats.map((_, i) => { const p = pt(i, 1); return <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke={colors.border.default} strokeWidth="1" />; })}
      <polygon points={stats.map((s, i) => { const p = pt(i, Math.max(s.value, 0.05)); return `${p.x},${p.y}`; }).join(" ")} fill="#f9731625" stroke={colors.accent.orange} strokeWidth="1.5" />
      {stats.map((s, i) => { const p = pt(i, Math.max(s.value, 0.05)); return <circle key={i} cx={p.x} cy={p.y} r="3" fill={colors.accent.orange} />; })}
      {stats.map((s, i) => { const lp = pt(i, 1.28); return <text key={i} x={lp.x} y={lp.y} textAnchor="middle" dominantBaseline="middle" fill={colors.text.dim} fontSize="9" fontFamily="Inter,sans-serif">{s.label}</text>; })}
    </svg>
  );
}
...
export default function DashboardRecruteur() {
  ...
  const st = {
    ...
    navbar: { background: colors.background.surface, borderBottom: "1px solid #222", padding: "0 2rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: "64px" },
    logo: { color: colors.accent.orange, fontWeight: 700, fontSize: "1.2rem", letterSpacing: "1px" },
    logoutBtn: { background: "transparent", border: "1px solid #333", color: colors.text.secondary, padding: "6px 16px", borderRadius: "8px", cursor: "pointer", fontSize: "13px" },
    content: { padding: "2rem", maxWidth: "1200px", margin: "0 auto" },
    tabs: { display: "flex", gap: "8px", marginBottom: "2rem", flexWrap: "wrap" },
    tab: (active) => ({ padding: "10px 24px", borderRadius: "8px", border: active ? "none" : "1px solid #333", background: active ? colors.accent.orange : "transparent", color: active ? colors.black : colors.text.secondary, fontWeight: active ? 600 : 400, cursor: "pointer", fontSize: "14px" }),
    filterBar: { background: colors.background.surface, border: "1px solid #222", borderRadius: "12px", padding: "1.25rem", marginBottom: "1.5rem", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "12px" },
    filterLabel: { fontSize: "11px", color: colors.text.dim, textTransform: "uppercase", letterSpacing: "0.5px" },
    select: { background: colors.background.raised, border: "1px solid #333", borderRadius: "8px", color: colors.text.primary, padding: "8px 12px", fontSize: "13px" },
    searchInput: { background: colors.background.raised, border: "1px solid #333", borderRadius: "8px", color: colors.text.primary, padding: "8px 12px", fontSize: "13px", width: "100%", boxSizing: "border-box" },
    grid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "16px" },
    card: { background: colors.background.surface, border: "1px solid #222", borderRadius: "12px", padding: "1.25rem" },
    avatar: { width: "44px", height: "44px", borderRadius: "50%", background: "#1a2e1a", display: "flex", alignItems: "center", justifyContent: "center", color: colors.accent.orange, fontWeight: 700, fontSize: "16px" },
    ...
    statVal: { fontSize: "18px", fontWeight: 700, color: colors.accent.orange },
    statLabel: { fontSize: "10px", color: colors.text.faint, textTransform: "uppercase" },
    cardActions: { display: "flex", gap: "8px", marginTop: "12px" },
    btnPrimary: { flex: 1, background: colors.accent.orange, color: colors.black, border: "none", borderRadius: "8px", padding: "8px", cursor: "pointer", fontWeight: 600, fontSize: "13px" },
    btnSecondary: { background: "transparent", border: "1px solid #333", color: colors.text.secondary, borderRadius: "8px", padding: "8px 12px", cursor: "pointer", fontSize: "13px" },
    favoriBtnActive: { background: "transparent", border: "1px solid #f97316", color: colors.accent.orange, borderRadius: "8px", padding: "8px 12px", cursor: "pointer", fontSize: "13px" },
    ...
    modal: { background: colors.background.surface, border: "1px solid #333", borderRadius: "16px", padding: "2rem", maxWidth: "600px", width: "100%", maxHeight: "90vh", overflowY: "auto" },
    sectionTitle: { fontSize: "11px", color: colors.accent.orange, textTransform: "uppercase", letterSpacing: "1px", margin: "1.5rem 0 0.75rem" },
    ...
    statBoxVal: { fontSize: "22px", fontWeight: 700, color: colors.accent.orange },
    statBoxLabel: { fontSize: "11px", color: colors.text.faint, marginTop: "2px" },
    textarea: { width: "100%", background: colors.background.raised, border: "1px solid #333", borderRadius: "8px", color: colors.text.primary, padding: "12px", fontSize: "14px", resize: "vertical", minHeight: "120px", boxSizing: "border-box", marginTop: "8px", fontFamily: "inherit" },
    emptyState: { textAlign: "center", padding: "4rem 2rem", color: colors.text.disabled },
    videoBox: { background: colors.background.raised, borderRadius: "8px", padding: "1rem", marginTop: "8px" },
    msgBubble: (mine) => ({ maxWidth: "70%", padding: "10px 14px", borderRadius: mine ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: mine ? colors.accent.orange : colors.background.raised, color: mine ? colors.black : colors.text.primary, fontSize: "14px", alignSelf: mine ? "flex-end" : "flex-start", marginBottom: "8px" }),
  };
        ...
        <div style={{ ...st.content, maxWidth: "700px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "1.5rem", marginBottom: "2rem", flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", marginBottom: "6px" }}>
                ...
                <span style={{ background: colors.accent.orange + alpha.soft, color: colors.accent.orange, fontSize: "11px", fontWeight: 700, padding: "2px 8px", borderRadius: "20px" }}>PRO</span>
                ...
                {isNouveau(j) && <span style={{ background: colors.accent.orange, color: colors.black, fontSize: "10px", fontWeight: 700, padding: "2px 8px", borderRadius: "20px" }}>NEW</span>}
                {aEteContacte(j.id) && <span style={{ background: colors.accent.blue + alpha.soft, color: colors.accent.blue, fontSize: "11px", padding: "2px 8px", borderRadius: "20px", border: "1px solid #60a5fa40" }}>✓ Contacté</span>}
              </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "2rem", marginBottom: "1.5rem", background: colors.background.surface, border: "1px solid #1e1e1e", borderRadius: "12px", padding: "1.5rem", flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minWidth: "160px" }}>
              ...
              {[
                { label: "Buts", val: j.buts_total || 0, max: 20, color: colors.accent.orange },
                { label: "Passes déc.", val: j.passes_decisives || 0, max: 15, color: colors.accent.orange },
                { label: "Matchs officiels", val: j.matchs_officiel || 0, max: 30, color: colors.accent.blue },
                { label: "Clean sheets", val: j.cleansheets || 0, max: 10, color: colors.accent.purple },
              ].map(s => (
                <div key={s.label} style={{ marginBottom: "10px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "3px" }}>
                    <span style={{ fontSize: "12px", color: colors.text.dim }}>{s.label}</span>
          ...
          {j.niveau_equipe && (
            <>
              ...
              <div style={{ background: colors.background.raised, borderRadius: "8px", padding: "1rem" }}>
                ...
                <p style={{ margin: "4px 0 0", fontSize: "13px", color: colors.text.dim }}>{j.niveau_equipe}</p>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "8px" }}>
                {j.points_forts.split(", ").filter(Boolean).map(tag => (
                  <span key={tag} style={{ background: colors.accent.orange + alpha.soft, color: colors.accent.orange, border: "1px solid #f9731640", fontSize: "12px", fontWeight: 600, padding: "4px 12px", borderRadius: "20px" }}>{tag}</span>
                ))}
              </div>
            </>
          )}
          ...
          {joueurParcours.length > 0 && (
            <>
              ...
              <div style={{ background: colors.background.surface, border: "1px solid #1e1e1e", borderRadius: "12px", padding: "1.25rem" }}>
                {joueurParcours.map((p, i) => (
                  <div key={p.id} style={{ display: "flex", gap: "14px" }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
                      <div style={{ width: "9px", height: "9px", borderRadius: "50%", background: colors.accent.orange, marginTop: "5px", flexShrink: 0 }} />
                      ...
                      <div>
                        ...
                        {(p.matchs_joues > 0 || p.buts > 0 || p.passes_decisives > 0 || p.cleansheets > 0) && (
                          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                            {p.matchs_joues > 0 && <span style={{ fontSize: "11px", color: colors.accent.orange }}>⚽ {p.matchs_joues} matchs</span>}
              ...
              <div style={st.videoBox}>
                {isVeo(videoUrl) || isYoutube(videoUrl) ? (
                  <a href={videoUrl} target="_blank" rel="noreferrer"
                    style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: colors.accent.orange + alpha.subtle, border: "1px solid #f9731640", color: colors.accent.orange, padding: "10px 20px", borderRadius: "8px", fontSize: "14px", fontWeight: 600, textDecoration: "none" }}>
                    🎬 {isVeo(videoUrl) ? "Ouvrir sur Veo" : "Ouvrir sur YouTube"}
                  </a>
                ) : isCloudinary(videoUrl) ? (
                  <>
                    <video src={videoUrl} controls style={{ width: "100%", borderRadius: "8px", maxHeight: "300px", background: colors.black }} />
                    <a href={videoUrl} target="_blank" rel="noreferrer" style={{ display: "block", marginTop: "8px", color: colors.accent.orange, fontSize: "12px", textDecoration: "none" }}>
                      🔗 Ouvrir dans un nouvel onglet
                    </a>
                  </>
                ) : (
                  <a href={videoUrl} target="_blank" rel="noreferrer" style={{ color: colors.accent.orange, fontSize: "14px", textDecoration: "none" }}>🎬 Voir la vidéo →</a>
                )}
              </div>
            </>
          )}
          ...
          <div style={{ marginTop: "2rem", background: "#0d1a0d", border: "1px solid #1e3a1e", borderRadius: "14px", padding: "1.5rem" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
              <div>
                <p style={{ margin: 0, fontWeight: 700, fontSize: "15px", color: colors.accent.orange }}>✅ Notes de saison validées par votre club</p>
              ...
              <button
                ...
                style={{ background: colors.accent.orange, color: colors.black, border: "none", padding: "10px 18px", borderRadius: "10px", fontSize: "13px", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}
              >+ Valider une note</button>
            </div>

            {validationsClub.length === 0 ? (
              <p style={{ margin: "1rem 0 0", fontSize: "13px", color: colors.text.disabled, textAlign: "center" }}>Aucune note validée pour ce joueur</p>
            ) : (
              <div style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "10px" }}>
                {validationsClub.map(v => (
                  <div key={v.id} style={{ background: colors.background.surface, border: "1px solid #1e2e1e", borderRadius: "10px", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px" }}>
                    <div>
          ...
          {showValidationModal && (
            <div onClick={() => setShowValidationModal(false)} style={{ position: "fixed", inset: 0, background: "#000000bb", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
              <div onClick={e => e.stopPropagation()} style={{ background: colors.background.surface, border: "1px solid #2a2a2a", borderRadius: "16px", padding: "2rem", maxWidth: "480px", width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
                <h3 style={{ margin: "0 0 4px", fontSize: "18px", fontWeight: 800 }}>Valider une note de saison</h3>
                <p style={{ margin: "0 0 1.5rem", fontSize: "13px", color: colors.text.dim }}>{j.prenom} {j.nom}</p>

                {/* Saison */}
                <label style={{ fontSize: "12px", color: colors.text.secondary, display: "block", marginBottom: "6px" }}>Saison</label>
                <select value={valSaison} onChange={e => setValSaison(e.target.value)} style={{ width: "100%", background: colors.background.raised, border: "1px solid #333", borderRadius: "8px", color: colors.text.primary, padding: "10px 12px", fontSize: "14px", marginBottom: "1.25rem", boxSizing: "border-box" }}>
                ...
                {valNote > 0 && <p style={{ margin: "-0.75rem 0 1.25rem", fontSize: "13px", color: colors.text.dim }}>{['','Très insuffisant','Insuffisant','Bien','Très bien','Excellent'][valNote]}</p>}
                ...
                <div style={{ display: "flex", gap: "8px", marginBottom: "1.25rem" }}>
                  {[{ val: 'feuilles', label: '📄 5 feuilles de match' }, { val: 'licence', label: '🪪 Numéro de licence' }].map(opt => (
                    <button key={opt.val} onClick={() => setValJustif(opt.val)} style={{ flex: 1, background: valJustif === opt.val ? colors.accent.orange + alpha.subtle : colors.background.raised, border: `1px solid ${valJustif === opt.val ? colors.accent.orange : colors.border.strong}`, color: valJustif === opt.val ? colors.accent.orange : colors.text.secondary, padding: "10px", borderRadius: "8px", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>
                      {opt.label}
                    </button>
                  ))}
                </div>
                ...
                {valJustif === 'feuilles' && (
                  <div style={{ marginBottom: "1.25rem" }}>
                    <p style={{ margin: "0 0 10px", fontSize: "12px", color: colors.text.dim }}>Upload tes 5 feuilles de match (photo ou PDF)</p>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                      {valFeuilles.map((url, i) => (
                        <label key={i} style={{ cursor: "pointer", gridColumn: i === 4 ? "1 / -1" : "auto" }}>
                          <div style={{ border: "2px dashed #2a2a2a", borderRadius: "10px", padding: "14px 10px", textAlign: "center", background: url ? colors.accent.orange + alpha.faint : colors.background.raised, borderColor: url ? "#f9731660" : colors.border.default, transition: "all 0.2s", minHeight: "80px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                            {valFeuillesUploading[i] ? (
                              <p style={{ margin: 0, fontSize: "12px", color: colors.accent.orange }}>⏳ Upload...</p>
                            ) : url ? (
                              <div style={{ width: "100%" }}>
                                ...
                                <p style={{ margin: "4px 0 0", fontSize: "10px", color: colors.accent.orange, fontWeight: 700 }}>✓ Feuille #{i+1}</p>
                                <p style={{ margin: "2px 0 0", fontSize: "10px", color: colors.text.disabled }}>Changer</p>
                              </div>
                            ) : (
                              <div>
                                <p style={{ margin: "0 0 4px", fontSize: "20px" }}>📋</p>
                                <p style={{ margin: 0, fontSize: "11px", color: colors.text.disabled, fontWeight: 600 }}>Feuille #{i+1}</p>
                ...
                {valJustif === 'licence' && (
                  <div style={{ marginBottom: "1.25rem" }}>
                    ...
                    <label style={{ display: "block", cursor: "pointer" }}>
                      <div style={{ border: "2px dashed #2a2a2a", borderRadius: "10px", padding: "20px", textAlign: "center", background: valLicenceUrl ? colors.accent.orange + alpha.faint : colors.background.raised, borderColor: valLicenceUrl ? "#f9731660" : colors.border.default, transition: "all 0.2s" }}>
                        {valLicenceUploading ? (
                          <p style={{ margin: 0, fontSize: "13px", color: colors.accent.orange }}>⏳ Upload en cours...</p>
                        ) : valLicenceUrl ? (
                          <div>
                            <p style={{ margin: "0 0 6px", fontSize: "13px", color: colors.accent.orange, fontWeight: 700 }}>✓ Document uploadé</p>
                    ...
                    <div style={{ margin: "12px 0 4px", display: "flex", alignItems: "center", gap: "8px" }}>
                      ...
                      <span style={{ fontSize: "11px", color: colors.text.disabled }}>ou renseigne le numéro</span>
                      <div style={{ flex: 1, height: "1px", background: colors.border.default }} />
                    </div>
                    ...
                    <input
                      ...
                      style={{ width: "100%", background: colors.background.raised, border: "1px solid #333", borderRadius: "8px", color: colors.text.primary, padding: "10px 12px", fontSize: "13px", outline: "none", boxSizing: "border-box", marginTop: "4px" }}
                    />
                  </div>
                )}
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  <button
                    ...
                    style={{ flex: 1, background: colors.accent.orange, color: colors.black, border: "none", padding: "12px", borderRadius: "10px", fontSize: "14px", fontWeight: 700, cursor: "pointer", opacity: (valSending || valLicenceUploading || valFeuillesUploading.some(Boolean) || !valNote || (valJustif === 'feuilles' && valFeuilles.filter(f => f.trim()).length < 5) || (valJustif === 'licence' && !valLicence.trim() && !valLicenceUrl)) ? 0.4 : 1 }}
                  >{valSending ? "Validation..." : "✅ Valider cette note"}</button>
                  <button onClick={() => setShowValidationModal(false)} style={{ background: colors.background.raised, color: colors.text.dim, border: "1px solid #2a2a2a", padding: "12px 20px", borderRadius: "10px", fontSize: "14px", cursor: "pointer" }}>Annuler</button>
                </div>
              </div>
            </div>
          )}
    ...
    <div style={st.page}>
      ...
      <nav style={st.navbar}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          <button onClick={() => setActiveTab("joueurs")} style={{ position: "relative", background: "transparent", border: "none", cursor: "pointer", fontSize: "20px", padding: "4px 8px" }} title={`${nbNouveaux} nouveau${nbNouveaux > 1 ? "x" : ""} joueur${nbNouveaux > 1 ? "s" : ""} cette semaine`}>
            🔔
            {nbNouveaux > 0 && (
              <span style={{ position: "absolute", top: "0", right: "0", background: colors.accent.red, color: colors.text.primary, borderRadius: "50%", width: "16px", height: "16px", fontSize: "10px", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{nbNouveaux}</span>
            )}
          </button>
      ...
      <div style={st.content}>
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <p style={{ margin: 0, color: colors.text.faint, fontSize: "13px" }}>{joueurs.length} joueur{joueurs.length !== 1 ? "s" : ""} Pro actif{joueurs.length !== 1 ? "s" : ""}{nbNouveaux > 0 ? ` · ${nbNouveaux} nouveau${nbNouveaux > 1 ? "x" : ""} cette semaine` : ""}</p>
        </div>
          ...
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px", marginBottom: "2rem" }}>
              {[
                ...
                <div key={s.label} style={{ background: colors.background.surface, border: "1px solid #222", borderRadius: "12px", padding: "1.5rem", textAlign: "center" }}>
                  <p style={{ fontSize: "32px", fontWeight: 800, color: colors.accent.orange, margin: "0 0 4px" }}>{s.val}</p>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              <button onClick={() => navigate("/jogabonito")}
                ...
                <p style={{ fontSize: "13px", color: colors.text.dim, margin: 0 }}>Reels courts des joueurs</p>
              </button>
              <button onClick={() => navigate("/feed")}
                style={{ background: colors.background.surface, border: "1px solid #f9731640", borderRadius: "12px", padding: "2rem", cursor: "pointer", textAlign: "center", color: colors.text.primary }}>
                <p style={{ fontSize: "2.5rem", margin: "0 0 8px" }}>📋</p>
                <p style={{ fontWeight: 700, fontSize: "16px", margin: "0 0 4px" }}>Feed Scout</p>
                <p style={{ fontSize: "13px", color: colors.text.dim, margin: 0 }}>Clips + stats des joueurs Pro</p>
              </button>
            </div>
          </div>
        )}
        ...
        {activeTab === "joueurs" && (
          <>
            ...
            <div style={{ display: "flex", gap: "8px", marginBottom: "1rem", flexWrap: "wrap" }}>
              {POSTE_PILLS.map(p => (
                <button key={p.val} onClick={() => { setPoste(p.val); setStyleDeJeu("Tous"); }}
                  style={{ padding: "7px 14px", borderRadius: "20px", border: poste === p.val ? "none" : "1px solid #333", background: poste === p.val ? posteColor(p.val === "Tous" ? "" : p.val).text : "transparent", color: poste === p.val ? colors.black : colors.text.secondary, fontWeight: poste === p.val ? 700 : 400, cursor: "pointer", fontSize: "13px", display: "flex", alignItems: "center", gap: "5px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                <p style={{ fontSize: "13px", color: colors.text.dim, margin: 0 }}>{filtered.length} joueur{filtered.length !== 1 ? "s" : ""}</p>
                {(poste !== "Tous" || categorie !== "Toutes" || pied !== "Tous" || region !== "Toutes" || styleDeJeu !== "Tous" || ville || search) && (
                  <button onClick={() => { setPoste("Tous"); setCategorie("Toutes"); setPied("Tous"); setRegion("Toutes"); setStyleDeJeu("Tous"); setVille(""); setSearch(""); }}
                    style={{ background: "transparent", border: "1px solid #333", color: colors.text.secondary, padding: "3px 10px", borderRadius: "6px", fontSize: "11px", cursor: "pointer" }}>
                    ✕ Réinitialiser
                  </button>
                )}
              </div>
              <div style={{ display: "flex", gap: "4px" }}>
                {["grille", "liste"].map(m => (
                  <button key={m} onClick={() => setModeAffichage(m)}
                    style={{ background: modeAffichage === m ? colors.accent.orange : "transparent", border: "1px solid #333", color: modeAffichage === m ? colors.black : colors.text.dim, padding: "5px 10px", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>
                    {m === "grille" ? "⊞" : "☰"}
                  </button>
                ))}
              </div>
                ...
                {filtered.map(j => (
                  <div key={j.id} style={st.card}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        ...
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                            ...
                            {isNouveau(j) && <span style={{ background: colors.accent.orange, color: colors.black, fontSize: "10px", fontWeight: 700, padding: "1px 6px", borderRadius: "20px" }}>NEW</span>}
                            {aEteContacte(j.id) && <span style={{ background: colors.accent.blue + alpha.soft, color: colors.accent.blue, fontSize: "10px", padding: "1px 6px", borderRadius: "20px", border: "1px solid #60a5fa40" }}>Contacté</span>}
                          ...
                          <p style={{ fontSize: "12px", color: colors.text.dim, margin: "2px 0 0" }}>{j.categorie || "—"} · {j.region || "—"}</p>
                          {j.style_de_jeu && <span style={{ fontSize: "10px", color: colors.accent.blue, background: "#60a5fa10", border: "1px solid #60a5fa30", padding: "1px 7px", borderRadius: "20px", display: "inline-block", marginTop: "3px" }}>⚡ {j.style_de_jeu}</span>}
                        </div>
                      </div>
                      {j.poste && <span style={st.posteBadge(j.poste)}>{j.poste}</span>}
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "12px" }}>
                      {[
                        { label: "Buts", val: j.buts_total || 0, max: maxButs, color: colors.accent.orange },
                        { label: "Passes déc.", val: j.passes_decisives || 0, max: maxPasses, color: colors.accent.orange },
                        { label: "Matchs", val: j.matchs_officiel || 0, max: maxMatchs, color: colors.accent.blue },
                      ].map(s => (
                        <div key={s.label}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "3px" }}>
                            <span style={{ fontSize: "11px", color: colors.text.faint }}>{s.label}</span>
                            <span style={{ fontSize: "11px", fontWeight: 700, color: s.color }}>{s.val}</span>
                          </div>
                          <div style={{ background: colors.background.raised, borderRadius: "4px", height: "4px" }}>
                    ...
                    {j.clip_url && <p style={{ fontSize: "11px", color: colors.accent.orange, margin: "0 0 10px" }}>🎬 Vidéo disponible</p>}
                    <div style={st.cardActions}>
                      <button style={st.btnPrimary} onClick={() => ouvrirProfilJoueur(j)}>Profil</button>
                      <button
                        style={isFavori(j.id) ? { ...st.favoriBtnActive, background: colors.accent.orange + alpha.subtle } : st.btnSecondary}
                  ...
                  <div key={j.id} style={{ ...st.card, padding: "12px 16px", display: "flex", alignItems: "center", gap: "14px" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                        ...
                        {isNouveau(j) && <span style={{ background: colors.accent.orange, color: colors.black, fontSize: "9px", fontWeight: 700, padding: "1px 5px", borderRadius: "20px" }}>NEW</span>}
                    ...
                    <div style={{ display: "flex", gap: "20px", fontSize: "12px", color: colors.text.dim }}>
                      ...
                      <span style={{ color: colors.accent.orange, fontWeight: 700 }}>{j.passes_decisives || 0} <span style={{ color: colors.text.faint, fontWeight: 400 }}>passes</span></span>
                      <span style={{ color: colors.accent.blue, fontWeight: 700 }}>{j.matchs_officiel || 0} <span style={{ color: colors.text.faint, fontWeight: 400 }}>matchs</span></span>
                    </div>
        ...
        {activeTab === "favoris" && (
          <div>
            ...
            <div style={{ display: "flex", gap: "8px", marginBottom: "1.5rem", flexWrap: "wrap" }}>
              {["Tous", ...dossiers].map(d => (
                <button key={d} onClick={() => setDossierActif(d)}
                  style={{ padding: "6px 14px", borderRadius: "20px", border: dossierActif === d ? "none" : "1px solid #333", background: dossierActif === d ? colors.accent.orange : "transparent", color: dossierActif === d ? colors.black : colors.text.secondary, fontWeight: dossierActif === d ? 600 : 400, cursor: "pointer", fontSize: "13px" }}>
            ...
            {favoris.length === 0 ? (
              <div style={st.emptyState}><p style={{ fontSize: "2rem" }}>☆</p><p>Aucun favori pour l'instant.<br /><span style={{ fontSize: "13px", color: colors.text.disabled }}>Cliquez sur ☆ depuis la liste des joueurs.</span></p></div>
            ) : (
              <div style={st.grid}>
                    ...
                    <div key={j.id} style={{ ...st.card, border: "1px solid #1a3a1a" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "10px" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <p style={{ fontSize: "12px", color: colors.text.dim, margin: 0 }}>{j.poste}{j.categorie ? ` · ${j.categorie}` : ""}</p>
                        </div>
                        <span style={{ background: colors.accent.orange + alpha.soft, color: colors.accent.orange, fontSize: "11px", padding: "2px 8px", borderRadius: "20px", flexShrink: 0 }}>{fav?.dossier || "Général"}</span>
                      </div>
                      ...
                      <div style={st.cardActions}>
                        ...
                        <button style={st.btnSecondary} onClick={() => setMessageModal(j)}>✉️</button>
                        <button style={{ ...st.btnSecondary, color: colors.accent.red, borderColor: colors.accent.red + alpha.medium }} onClick={() => toggleFavori(j.id)}>✕</button>
                      </div>
                    </div>
                  );
                })}
                        ...
                        <div onClick={() => ouvrirProfilJoueur(j)} style={{ borderRadius: "8px", overflow: "hidden", marginBottom: "12px", cursor: "pointer", position: "relative", aspectRatio: "16/9", background: colors.background.raised, display: "flex", alignItems: "center", justifyContent: "center" }}>
                          {thumb ? (
                            ...
                            <div style={{ textAlign: "center" }}>
                              ...
                              <p style={{ margin: 0, fontSize: "11px", color: colors.accent.orange, fontWeight: 700, letterSpacing: "1px" }}>VEO</p>
                            </div>
                          ) : isYtUrl ? (
                            <div style={{ textAlign: "center" }}>
                              <p style={{ margin: "0 0 4px", fontSize: "1.8rem" }}>▶️</p>
                              <p style={{ margin: 0, fontSize: "11px", color: colors.accent.red, fontWeight: 700 }}>YouTube</p>
                            </div>
                          ) : (
        ...
        {activeTab === "messages" && (
          <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: "16px", minHeight: "500px" }}>
            ...
            <div style={{ background: colors.background.surface, border: "1px solid #222", borderRadius: "12px", overflow: "hidden" }}>
              <div style={{ padding: "1rem", borderBottom: "1px solid #222" }}>
                <p style={{ margin: 0, fontWeight: 600, color: colors.accent.orange, fontSize: "14px" }}>💬 Conversations</p>
              </div>
                ...
                conversations.map(conv => (
                  <div key={conv.otherId} onClick={() => ouvrirConversation(conv)}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <div style={{ width: "30px", height: "30px", borderRadius: "50%", background: "#1a2e1a", display: "flex", alignItems: "center", justifyContent: "center", color: colors.accent.orange, fontWeight: 700, fontSize: "11px", flexShrink: 0 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        <p style={{ margin: "1px 0 0", fontSize: "11px", color: conv.other?.plan === "coach" ? colors.accent.orange : colors.accent.orange }}>
            ...
            {/* Zone de chat */}
            <div style={{ background: colors.background.surface, border: "1px solid #222", borderRadius: "12px", display: "flex", flexDirection: "column" }}>
              {convActive ? (
                <>
                  <div style={{ padding: "1rem", borderBottom: "1px solid #222", display: "flex", alignItems: "center", gap: "12px" }}>
                    <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "#1a2e1a", display: "flex", alignItems: "center", justifyContent: "center", color: colors.accent.orange, fontWeight: 700, fontSize: "13px" }}>
                    ...
                    <div>
                      ...
                      <p style={{ margin: 0, fontSize: "12px", color: convActive.other?.plan === "coach" ? colors.accent.orange : colors.accent.orange }}>
                  ...
                  <div style={{ padding: "1rem", borderTop: "1px solid #222", display: "flex", gap: "8px" }}>
                    <input
                      style={{ flex: 1, background: colors.background.raised, border: "1px solid #333", borderRadius: "8px", color: colors.text.primary, padding: "10px 12px", fontSize: "14px", outline: "none" }}
                      placeholder="Écrire un message..."
                      value={newMessage}
                      onChange={e => setNewMessage(e.target.value)}
                      onKeyDown={e => e.key === "Enter" && envoyerMessage()}
                    />
                    <button onClick={envoyerMessage} style={{ background: colors.accent.orange, color: colors.black, border: "none", padding: "10px 18px", borderRadius: "8px", fontWeight: 700, cursor: "pointer" }}>Envoyer</button>
                  </div>
                </>
              ) : (
            ...
            <div style={{ background: colors.background.surface, border: "1px solid #1a1a1a", borderRadius: "16px", padding: "24px", marginBottom: "16px", display: "flex", alignItems: "center", gap: "20px" }}>
              <div style={{ position: "relative", flexShrink: 0 }}>
                {recruteur?.avatar_url
                  ? <img src={recruteur.avatar_url} alt="" style={{ width: "72px", height: "72px", borderRadius: "50%", objectFit: "cover", border: "2px solid #f9731640" }} />
                  : <div style={{ width: "72px", height: "72px", borderRadius: "50%", background: "#1a2e1a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "26px", fontWeight: 800, color: colors.accent.orange }}>
                      {(profilEdit.prenom || "?")[0]}{(profilEdit.nom || "?")[0]}
                    </div>
                }
                <label style={{ position: "absolute", bottom: 0, right: 0, width: "24px", height: "24px", background: colors.accent.orange, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: avatarUploading ? "wait" : "pointer", border: "2px solid #0a0a0a", fontSize: "11px" }}>
                  {avatarUploading ? "…" : "✎"}
              ...
              <div>
                ...
                <p style={{ fontSize: "13px", color: colors.accent.orange, margin: 0 }}>{profilEdit.type_recruteur || "Recruteur"} · {profilEdit.club || profilEdit.region || "—"}</p>
              </div>
            </div>

            {/* Formulaire */}
            <div style={{ background: colors.background.surface, border: "1px solid #1a1a1a", borderRadius: "16px", padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <div>
                <label style={{ fontSize: "11px", color: colors.text.faint, textTransform: "uppercase", letterSpacing: "0.5px", display: "block", marginBottom: "6px" }}>Type de recruteur</label>
              ...
              <button onClick={handleSaveProfil} disabled={savingProfil}
                style={{ background: savingProfil ? colors.border.strong : colors.accent.orange, color: savingProfil ? colors.text.dim : colors.black, border: "none", padding: "12px 28px", borderRadius: "10px", fontSize: "14px", fontWeight: 700, cursor: savingProfil ? "not-allowed" : "pointer", alignSelf: "flex-start" }}>
      ...
      {messageModal && (
        <div style={st.overlay} onClick={() => setMessageModal(null)}>
          <div style={st.modal} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: "13px", color: colors.text.dim, margin: "0 0 1.5rem" }}>{messageModal.poste} · {messageModal.categorie}</p>
            {messageSent ? (
              <div style={{ textAlign: "center", padding: "2rem", color: colors.accent.orange }}><p style={{ fontSize: "2rem" }}>✓</p><p>Message envoyé !</p></div>
            ) : (
              <>
...
function CoachContact({ coaches, recruteurId, contacterCoach, chargerConversations, setActiveTab, ouvrirConversation, conversations, st }) {
  ...
  if (coaches.length === 0) {
    return (
      <div style={{ background: colors.background.surface, border: "1px dashed #333", borderRadius: "12px", padding: "3rem", textAlign: "center", color: colors.text.faint }}>
  ...
  return (
    <div style={{ maxWidth: "600px" }}>
      <div style={{ background: colors.background.surface, border: "1px solid #222", borderRadius: "12px", padding: "1.5rem", marginBottom: "1.5rem" }}>
        <h2 style={{ margin: "0 0 4px", fontSize: "1.1rem", fontWeight: 700 }}>🎙️ Contacter notre coach expert</h2>
        <p style={{ margin: 0, fontSize: "13px", color: colors.text.dim }}>Posez vos questions sur un joueur ou demandez une collaboration.</p>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "1.5rem" }}>
        {coaches.map(coach => (
          <div key={coach.id}
            ...
            style={{ background: colors.background.surface, border: `1px solid ${selectedCoach?.id === coach.id ? colors.accent.orange : "#222"}`, borderRadius: "12px", padding: "1rem", cursor: "pointer", display: "flex", alignItems: "center", gap: "12px" }}>
            <div style={{ width: "44px", height: "44px", borderRadius: "50%", background: "#1a1a0a", display: "flex", alignItems: "center", justifyContent: "center", color: colors.accent.orange, fontWeight: 700, fontSize: "16px", flexShrink: 0 }}>
              {(coach.prenom || "C")[0]}{(coach.nom || "")[0]}
            </div>
            <div>
              <p style={{ margin: 0, fontWeight: 600 }}>{coach.prenom} {coach.nom}</p>
              <p style={{ margin: "2px 0 0", fontSize: "12px", color: colors.accent.orange }}>🎙️ Coach Expert</p>
              {coach.club && <p style={{ margin: "2px 0 0", fontSize: "12px", color: colors.text.dim }}>{coach.club}</p>}
            </div>
            {selectedCoach?.id === coach.id && <span style={{ marginLeft: "auto", color: colors.accent.orange, fontSize: "18px" }}>✓</span>}
          </div>
        ))}
      </div>
      ...
      {/* Message */}
      {selectedCoach && (
        <div style={{ background: colors.background.surface, border: "1px solid #222", borderRadius: "12px", padding: "1.5rem" }}>
          <label style={{ fontSize: "11px", color: colors.text.dim, textTransform: "uppercase", letterSpacing: "0.5px", display: "block", marginBottom: "8px" }}>
            Votre message à {selectedCoach.prenom}
          </label>
          {sent ? (
            <div style={{ textAlign: "center", padding: "2rem", color: colors.accent.orange }}>
              <p style={{ fontSize: "2rem" }}>✓</p>
            ...
            <>
              <textarea
                style={{ width: "100%", background: colors.background.raised, border: "1px solid #333", borderRadius: "8px", color: colors.text.primary, padding: "12px", fontSize: "14px", resize: "vertical", minHeight: "140px", boxSizing: "border-box", fontFamily: "inherit" }}
              ...
              <button
                ...
                style={{ marginTop: "12px", width: "100%", background: colors.accent.orange, color: colors.black, border: "none", borderRadius: "8px", padding: "12px", fontWeight: 700, fontSize: "14px", cursor: "pointer", opacity: (sending || !message.trim()) ? 0.6 : 1 }}>
