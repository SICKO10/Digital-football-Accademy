...
export default function Upload() {
  ...
  const [mode, setMode] = useState('lien') // 'lien' ou 'fichier'
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.container}>
        ...
        <p style={{ color: '#666', marginBottom: '2rem', fontSize: '14px' }}>Ton clip sera visible par les recruteurs dans le Scout Center</p>

        {success ? (
          <div style={{ ...s.box, border: '2px solid #4ade80', textAlign: 'center', padding: '3rem' }}>
            <p style={{ fontSize: '3rem', marginBottom: '1rem' }}>✅</p>
            <h2 style={{ fontSize: '1.4rem', fontWeight: 700, marginBottom: '0.5rem' }}>Vidéo partagée !</h2>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
              ...
              <button onClick={() => { setSuccess(false); setFile(null); setLien(''); setProgress(0) }} style={{ background: '#1a1a1a', border: '1px solid #333', color: '#aaa', padding: '10px 24px', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}>Partager un autre</button>
            </div>
          </div>
        ) : (
          <>
            ...
            >
              {uploading ? `En cours... ${mode === 'fichier' ? progress + '%' : ''}` : mode === 'lien' ? '🔗 Partager le lien' : '🚀 Envoyer ma vidéo'}
            </button>

            <p style={{ textAlign: 'center', fontSize: '12px', color: '#555', marginTop: '1rem' }}>
              Seuls les recruteurs abonnés peuvent voir tes vidéos
            </p>
          </>
        )}
      </div>
