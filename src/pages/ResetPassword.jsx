      ...
      <div style={{ background: colors.background.surface, border: '1px solid #222', borderRadius: '16px', padding: '2.5rem', width: '100%', maxWidth: '400px' }}>
        ...
        {done ? (
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ color: colors.text.dim, fontSize: '14px' }}>{t('auth_redirection_connexion', lang)}</p>
          </div>
        ) : erreurLien ? (
          <div style={{ textAlign: 'center' }}>
            <p style={{ color: '#ff4444', fontSize: '14px', marginBottom: '1.5rem' }}>{erreurLien}</p>
            <p style={{ fontSize: '12px', color: colors.text.disabled }}>
              <span onClick={() => navigate('/forgot-password')} style={{ color: colors.accent.green, cursor: 'pointer' }}>
          ...
          <>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1rem' }}>
              <div>
                <label style={{ fontSize: '13px', color: colors.text.secondary, display: 'block', marginBottom: '6px' }}>{t('auth_nouveau_mdp', lang)}</label>
                <input
                  type="password"
                  value={password}
