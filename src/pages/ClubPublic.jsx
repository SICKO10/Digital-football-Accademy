...
export default function ClubPublic() {
  ...
  if (loading) return <div style={{ ...st.page, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#4ade80' }}>{t('jexp_chargement', lang)}</div>
  if (!educateur) return <div style={{ ...st.page, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#555' }}>{t('clubpub_club_introuvable', lang)}</div>

  return (
    <div style={st.page}>
      <nav style={st.navbar}>
        <span style={{ fontWeight: 800, fontSize: '15px', letterSpacing: '1px' }}>⬡ DIGITAL FOOTBALL</span>
        <button onClick={() => navigate(-1)} style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', color: '#aaa', padding: '8px 16px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>{t('clubpub_retour', lang)}</button>
      </nav>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#555' }}>
              {t('clubpub_educateur_label', lang)} <span style={{ color: '#aaa' }}>{educateur.prenom} {educateur.nom}</span>
            </p>
          </div>
          {estValide && (
          ...
          <div>
            {categories.length === 0 ? (
              <div style={{ ...st.card, textAlign: 'center', padding: '48px', color: '#333' }}>
                ...
                <p style={{ color: '#444' }}>{t('clubpub_aucun_joueur', lang)}</p>
              </div>
            ) : categories.map(cat => {
        ...
        {onglet === 'resultats' && (
          <div>
            {matchs.length === 0 ? (
              <div style={{ ...st.card, textAlign: 'center', padding: '48px', color: '#333' }}>
                <p style={{ fontSize: '32px' }}>⚽</p>
                <p style={{ color: '#444' }}>{t('clubpub_aucun_resultat', lang)}</p>
              </div>
            ) : matchs.map(m => {
          ...
          <div>
            ...
            <div style={{ ...st.card, background: '#0d1a0d', border: '1px solid #1e3a1e' }}>
              ...
              {estValide ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '12px' }}>
                    {validations.map(v => (
                      <div key={v.id} style={{ background: '#111', borderRadius: '8px', padding: '8px 12px', display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                        ...
                        <span style={{ color: '#555' }}>📄 {(v.feuilles_match || []).length} {t('clubpub_feuilles_suffix', lang)}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <p style={{ margin: '0 0 12px', fontSize: '13px', color: '#aaa', lineHeight: 1.6 }}>
                    {t('clubpub_prouve_participation', lang)} <strong style={{ color: '#fff' }}>{t('clubpub_5_feuilles_match', lang)}</strong> {t('clubpub_liens_gdrive_etc', lang)}
                  </p>
              ...
              {showValForm && (
                <div style={{ marginTop: '16px', borderTop: '1px solid #1e3a1e', paddingTop: '16px' }}>
                  <label style={{ fontSize: '12px', color: '#aaa', display: 'block', marginBottom: '6px' }}>{t('clubpub_saison_label', lang)}</label>
                  ...
                  <label style={{ fontSize: '12px', color: '#aaa', display: 'block', marginBottom: '8px' }}>{t('clubpub_liens_feuilles_label', lang)}</label>
            ...
            {!estValide && !valSuccess ? (
              <div style={{ ...st.card, textAlign: 'center', padding: '40px', color: '#2a2a2a' }}>
                ...
                <p style={{ color: '#444', fontSize: '14px' }}>{t('clubpub_valide_pour_noter', lang)}</p>
              </div>
            ) : (
              <div style={st.card}>
                ...
                {noteDone ? (
                  <div style={{ textAlign: 'center', padding: '24px 0' }}>
                    ...
                    <p style={{ fontWeight: 700, color: '#4ade80' }}>{t('clubpub_note_envoyee', lang)}</p>
                  ...
                  <>
                    <div style={{ marginBottom: '1rem' }}>
                      ...
                      {noteVal > 0 && <p style={{ margin: '8px 0 0', fontSize: '13px', color: '#666' }}>{['', t('clubpub_note_tres_insuffisant', lang), t('clubpub_note_insuffisant', lang), t('clubpub_note_bien', lang), t('clubpub_note_tres_bien', lang), t('clubpub_note_excellent', lang)][noteVal]}</p>}
                    </div>
                    ...
                    {monAvis && <p style={{ margin: '8px 0 0', fontSize: '11px', color: '#555', textAlign: 'center' }}>{t('clubpub_deja_note', lang)} {monAvis.note}/5 — {t('clubpub_tu_peux_modifier', lang)}</p>}
                  </>
                )}
              </div>
            )}
