...
const TYPE_META = {
  mp4:       { label: 'Vidéo HD',   color: '#4ade80', emoji: '🎬' },
  ...
  link:      { label: 'Vidéo',      color: '#4ade80', emoji: '▶' },
}
...
function VideoPlayer({ url }) {
  ...
  if (type === 'youtube') {
    ...
    return (
      <div style={box}>
        <iframe src={`https://www.youtube.com/embed/${id}`} style={{ width: '100%', height: '100%', border: 'none' }}
  ...
  if (type === 'tiktok') {
    ...
    return (
      <div style={{ ...box, aspectRatio: '9/16', maxHeight: '480px' }}>
        <iframe src={`https://www.tiktok.com/embed/v2/${id}`} style={{ width: '100%', height: '100%', border: 'none' }}
          allow="encrypted-media" allowFullScreen title="TikTok" />
      </div>
    )
  }
...
function VideoCard({ j, user, profil, interactions, onRefresh, onOpenProfile, st }) {
  ...
  // Fermer le menu partage au clic extérieur
  useEffect(() => {
  ...
  const handleLike = async () => {
    if (!user) return
    if (liked) {
      await supabase.from('likes').delete().eq('user_id', user.id).eq('joueur_id', j.id)
    } else {
      await supabase.from('likes').insert({ user_id: user.id, joueur_id: j.id })
    }
    onRefresh()
  }
  ...
  const handleFavori = async () => {
    if (!user) return
    if (favori) {
      await supabase.from('video_favoris').delete().eq('user_id', user.id).eq('joueur_id', j.id)
    } else {
      await supabase.from('video_favoris').insert({ user_id: user.id, joueur_id: j.id })
    }
    onRefresh()
  }
  ...
  const handleRepost = async () => {
    if (!user) return
    if (reposted) {
      await supabase.from('reposts').delete().eq('user_id', user.id).eq('joueur_id', j.id)
    } else {
      await supabase.from('reposts').insert({ user_id: user.id, joueur_id: j.id })
    }
    onRefresh()
  }
  ...
  const handleShare = (platform) => {
    ...
    const text = encodeURIComponent(`🎬 Regarde la vidéo de ${j.prenom} ${j.nom} (${j.poste}) sur Digital Football !`)
    const links = {
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(`https://digital-football-accademy.vercel.app/feed`)
    setShareOpen(false)
  }
  ...
  return (
    <div style={{ background: '#111', border: '1px solid #222', borderRadius: '16px', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div
          onClick={() => onOpenProfile(j)}
          style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#4ade8015', border: '2px solid #4ade8040', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px', fontWeight: 700, color: '#4ade80', flexShrink: 0, cursor: 'pointer' }}>
          {j.prenom?.[0]}{j.nom?.[0]}
        </div>
        <div style={{ flex: 1 }}>
          <p
            onClick={() => onOpenProfile(j)}
            style={{ margin: 0, fontWeight: 700, fontSize: '15px', cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          ...
          <button
            onClick={() => onOpenProfile(j)}
            style={{ background: '#4ade8015', border: '1px solid #4ade8040', color: '#4ade80', padding: '5px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>
            Profil →
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '4px 8px', display: 'flex', alignItems: 'center', gap: '4px', borderBottom: '1px solid #1a1a1a' }}>
        ...
        {/* Like ❤️ */}
        <button style={actionBtn(liked, '#ef4444')} onClick={handleLike} title={user ? (liked ? 'Retirer le like' : 'Liker') : 'Connectez-vous'}>
          <span style={{ fontSize: '16px' }}>{liked ? '❤️' : '🤍'}</span>
          <span>{likeCount}</span>
        </button>

        {/* Commentaire 💬 */}
        <button style={actionBtn(showComments, '#60a5fa')} onClick={() => setShowComments(!showComments)} title="Commentaires">
          <span style={{ fontSize: '16px' }}>💬</span>
          <span>{commentCount}</span>
        </button>

        {/* Repost 🔁 */}
        <button style={actionBtn(reposted, '#4ade80')} onClick={handleRepost} title={user ? (reposted ? 'Annuler le repost' : 'Reposter sur votre feed') : 'Connectez-vous'}>
          <span style={{ fontSize: '16px' }}>{reposted ? '🔁' : '↩️'}</span>
          <span>{repostCount > 0 ? repostCount : ''}</span>
        </button>

        {/* Favori ⭐ */}
        <button style={actionBtn(favori, '#f59e0b')} onClick={handleFavori} title={user ? (favori ? 'Retirer des favoris' : 'Ajouter aux favoris') : 'Connectez-vous'}>
          <span style={{ fontSize: '16px' }}>{favori ? '⭐' : '☆'}</span>
        </button>

        {/* Partage 📤 */}
        <div style={{ marginLeft: 'auto', position: 'relative' }} ref={shareRef}>
          <button style={actionBtn(false, '#aaa')} onClick={() => setShareOpen(!shareOpen)} title="Partager">
            <span style={{ fontSize: '16px' }}>📤</span>
            <span style={{ fontSize: '12px' }}>Partager</span>
          </button>
          {shareOpen && (
            <div style={{ position: 'absolute', bottom: '40px', right: 0, background: '#1a1a1a', border: '1px solid #333', borderRadius: '12px', padding: '8px', zIndex: 100, minWidth: '180px', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
              {[
                ...
                <button key={id} onClick={() => handleShare(id)}
                  style={{ display: 'flex', alignItems: 'center', gap: '10px', width: '100%', background: 'transparent', border: 'none', color: '#fff', padding: '8px 12px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', textAlign: 'left' }}
              ...
              <button onClick={handleCopyLink}
                style={{ display: 'flex', alignItems: 'center', gap: '10px', width: '100%', background: 'transparent', border: 'none', color: '#aaa', padding: '8px 12px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}
                onMouseEnter={e => e.currentTarget.style.background = '#2a2a2a'}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
      ...
      {showComments && (
        <div style={{ padding: '12px 16px' }}>
          {comments.length === 0 ? (
            <p style={{ fontSize: '13px', color: '#555', margin: '0 0 12px' }}>Aucun commentaire. Soyez le premier !</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '12px', maxHeight: '200px', overflowY: 'auto' }}>
              {comments.map((c, i) => (
                <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                  <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: '#1a2e1a', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#4ade80', fontSize: '11px', fontWeight: 700, flexShrink: 0 }}>
                    {c.author?.prenom?.[0]}{c.author?.nom?.[0]}
                  </div>
                  <div style={{ background: '#1a1a1a', borderRadius: '10px', padding: '8px 12px', flex: 1 }}>
                    <p style={{ margin: '0 0 2px', fontSize: '12px', fontWeight: 600, color: '#4ade80' }}>
          ...
          {user ? (
            <div style={{ display: 'flex', gap: '8px' }}>
              <input
                ...
                style={{ flex: 1, background: '#1a1a1a', border: '1px solid #333', borderRadius: '8px', color: '#fff', padding: '8px 12px', fontSize: '13px', outline: 'none' }}
              />
              <button onClick={handleComment} disabled={sendingComment || !newComment.trim()}
                style={{ background: '#4ade80', color: '#000', border: 'none', padding: '8px 16px', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '13px', opacity: (!newComment.trim() || sendingComment) ? 0.5 : 1 }}>
...
function Feed() {
  ...
  const chargerInteractions = async (u, joueurs) => {
    ...
    // Comments count + contenu
    const { data: commentsData } = await supabase
      .from('comments')
      .select('*, author:profiles!comments_user_id_fkey(prenom, nom, plan)')
      .in('joueur_id', ids)
      .order('created_at', { ascending: true })
    const cc = {}
    const ac = {}
    commentsData?.forEach(c => {
      cc[c.joueur_id] = (cc[c.joueur_id] || 0) + 1
      if (!ac[c.joueur_id]) ac[c.joueur_id] = []
      ac[c.joueur_id].push(c)
    })
    setCommentCounts(cc)
    setAllComments(ac)
  ...
  const refresh = async () => {
    await chargerInteractions(user, joueursPro)
  }
  ...
  // Feed personnalisé : reposts de l'utilisateur en premier, puis reste
  const feedVideos = user
    ? [...joueursAvecClipFiltres.filter(j => repostIds.includes(j.id)), ...joueursAvecClipFiltres.filter(j => !repostIds.includes(j.id))]
    : joueursAvecClipFiltres
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: 'white', fontFamily: 'sans-serif' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 2rem', borderBottom: '1px solid #222', position: 'sticky', top: 0, background: '#0a0a0aee', zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {favoriIds.length > 0 && vue === 'videos' && (
            <button onClick={() => setVue('favoris')}
              style={{ background: '#f59e0b15', border: '1px solid #f59e0b40', color: '#f59e0b', padding: '6px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
              ⭐ Favoris ({favoriIds.length})
            </button>
          )}
          {user ? (
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '2rem 1rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
          ...
          <p style={{ color: '#666', fontSize: '13px', margin: 0 }}>{joueursAvecClip.length} vidéo{joueursAvecClip.length > 1 ? 's' : ''} disponible{joueursAvecClip.length > 1 ? 's' : ''}</p>
        </div>

        {/* TABS */}
        <div style={{ display: 'flex', gap: '8px', marginBottom: '1.5rem', justifyContent: 'center' }}>
          {[
            { id: 'videos', label: `🎬 Vidéos (${joueursAvecClip.length})` },
            { id: 'profils', label: `👤 Profils (${joueursPro.length})` },
            ...(favoriIds.length > 0 ? [{ id: 'favoris', label: `⭐ Favoris (${favoriIds.length})` }] : []),
          ].map(t => (
            <button key={t.id} onClick={() => setVue(t.id)}
              style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: 600, background: vue === t.id ? '#4ade80' : '#1a1a1a', color: vue === t.id ? '#000' : '#aaa' }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
          <select value={filtrePoste} onChange={e => setFiltrePoste(e.target.value)}
            style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: '8px', color: '#fff', padding: '8px 12px', fontSize: '13px' }}>
        ...
        {vue === 'videos' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {feedVideos.length === 0 ? (
              ...
              <VideoCard key={j.id} j={j} user={user} profil={profil} interactions={interactions} onRefresh={refresh} onOpenProfile={setJoueurModal} st={st} />
            ))}
          </div>
        )}
        ...
        {vue === 'favoris' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {joueursAvecClip.filter(j => favoriIds.includes(j.id)).length === 0 ? (
              <div style={{ textAlign: 'center', padding: '4rem', color: '#555' }}>
                <p style={{ fontSize: '3rem' }}>⭐</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px' }}>
            {joueursFiltres.length === 0 ? (
              <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '4rem', color: '#555' }}>
                <p style={{ fontSize: '2rem' }}>🔍</p><p>Aucun joueur trouvé</p>
              </div>
              ...
              <div key={j.id} style={{ background: '#111', border: '1px solid #222', borderRadius: '12px', padding: '1.25rem', cursor: 'pointer' }} onClick={() => setJoueurModal(j)}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '12px' }}>
                  <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: '#4ade8015', border: '2px solid #4ade8040', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', fontWeight: 700, color: '#4ade80' }}>
                    {j.prenom?.[0]}{j.nom?.[0]}
                  </div>
                  <div>
      ...
      {joueurModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
          onClick={() => setJoueurModal(null)}>
          <div style={{ background: '#111', border: '1px solid #333', borderRadius: '16px', padding: '2rem', maxWidth: '560px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '1.5rem' }}>
              {[['📍', 'Région', joueurModal.region], ['🦵', 'Pied', joueurModal.pied], ['🏟️', 'Niveau', joueurModal.niveau_equipe], ['🏆', 'Club', joueurModal.club]].filter(([,,v]) => v).map(([ico, label, val]) => (
                <div key={label} style={{ background: '#1a1a1a', borderRadius: '8px', padding: '10px 14px' }}>
                  <p style={{ margin: 0, fontSize: '11px', color: '#666' }}>{ico} {label}</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '1.5rem' }}>
              {[['Matchs', joueurModal.matchs_officiel ?? 0], ['Minutes', joueurModal.minutes_jouees ?? 0], ['Buts', joueurModal.buts_total ?? 0], ['Passes déc.', joueurModal.passes_decisives ?? 0], ['Amicaux', joueurModal.matchs_amical ?? 0], ['Clean sheets', joueurModal.cleansheets ?? 0]].map(([label, val]) => (
            ...
            <div style={{ display: 'flex', gap: '8px', fontSize: '13px', color: '#666', marginBottom: '1rem' }}>
              {(likeCounts[joueurModal.id] || 0) > 0 && <span>❤️ {likeCounts[joueurModal.id]} like{likeCounts[joueurModal.id] > 1 ? 's' : ''}</span>}
              {(commentCounts[joueurModal.id] || 0) > 0 && <span>💬 {commentCounts[joueurModal.id]} commentaire{commentCounts[joueurModal.id] > 1 ? 's' : ''}</span>}
            </div>

            {joueurModal.clip_url && (
              <div style={{ marginBottom: '1.5rem' }}>
                <p style={{ fontSize: '12px', color: '#4ade80', fontWeight: 600, marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Vidéo</p>
            ...
            {user && (
              <div style={{ display: 'flex', gap: '8px', paddingTop: '1rem', borderTop: '1px solid #222', flexWrap: 'wrap' }}>
                <button onClick={async () => { await (likedIds.includes(joueurModal.id) ? supabase.from('likes').delete().eq('user_id', user.id).eq('joueur_id', joueurModal.id) : supabase.from('likes').insert({ user_id: user.id, joueur_id: joueurModal.id })); refresh() }}
                  style={{ background: likedIds.includes(joueurModal.id) ? '#ef444420' : '#1a1a1a', border: `1px solid ${likedIds.includes(joueurModal.id) ? '#ef4444' : '#333'}`, color: likedIds.includes(joueurModal.id) ? '#ef4444' : '#aaa', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
                  {likedIds.includes(joueurModal.id) ? '❤️ Liké' : '🤍 Liker'}
                </button>
                <button onClick={async () => { await (favoriIds.includes(joueurModal.id) ? supabase.from('video_favoris').delete().eq('user_id', user.id).eq('joueur_id', joueurModal.id) : supabase.from('video_favoris').insert({ user_id: user.id, joueur_id: joueurModal.id })); refresh() }}
                  style={{ background: favoriIds.includes(joueurModal.id) ? '#f59e0b20' : '#1a1a1a', border: `1px solid ${favoriIds.includes(joueurModal.id) ? '#f59e0b' : '#333'}`, color: favoriIds.includes(joueurModal.id) ? '#f59e0b' : '#aaa', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
                  {favoriIds.includes(joueurModal.id) ? '⭐ Favori' : '☆ Favoris'}
                </button>
                {isRecruteur && (
                  <button onClick={() => navigate('/club')}
                    style={{ background: '#4ade80', color: '#000', border: 'none', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
                    Scout Center →
                  </button>
                )}
              </div>
