-- Script unique : corrige les tables tournois_* créées avec l'ancien schéma
-- (club_id TEXT au lieu de UUID) puis les recrée avec le bon typage.
...
BEGIN
  SELECT
    COALESCE((SELECT count(*) FROM tournois_participation), 0)
    + COALESCE((SELECT count(*) FROM tournois_matchs), 0)
    + COALESCE((SELECT count(*) FROM tournois_convocations), 0)
  INTO total;

  IF total > 0 THEN
    RAISE EXCEPTION 'Abandon : % ligne(s) existante(s) dans les tables tournois_* — rien n''a été supprimé. Préviens-moi avant de continuer.', total;
  END IF;
END $$;
...
CREATE TABLE tournois_participation (
...
CREATE TABLE tournois_convocations (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_tournois_participation_club ON tournois_participation(club_id);
CREATE INDEX idx_tournois_convocations_tournoi ON tournois_convocations(tournoi_id);
CREATE INDEX idx_tournois_matchs_tournoi ON tournois_matchs(tournoi_id);
